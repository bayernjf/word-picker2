...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = { limit: 0, requireFrq: false };
 ...
 * 例："a. 新的" → "adj. 新的"
 */
function normalizePosInTranslation(text: string): string {
  // 匹配义项开头或分号后的 "a. "，替换为 "adj. "
  // 注意：不全局裸替换，避免误伤正文里的 "a."
  return text.replace(/(^|；)a\. /g, "$1adj. ");
}

function buildExchangeMap(exchange: string): string[] {
...
const lemmaMap: Record<string, string> = {}; // 变形 -> 原词
