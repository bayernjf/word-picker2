...
export async function addWord(entry) {
  ...
  if (duplicateIndex !== -1) {
    ...
    // 合并并去重
    const normalizeContext = (value) => String(value || '').trim().replace(/\s+/g, ' ');
    const normalizeSourceLink = (ctx) => String(ctx?.sourceLink || ctx?.source_link || ctx?.sourceUrl || ctx?.source_url || '').trim();
    ...
    newContexts.forEach(newCtx => {
      const isDuplicate = mergedContexts.some(existingCtx => 
        normalizeContext(existingCtx.context) === normalizeContext(newCtx.context) && 
        normalizeSourceLink(existingCtx) === normalizeSourceLink(newCtx));
      if (!isDuplicate) {
